...
Based on CLT, the distribution of the mean of the sample follows the normal distribution. So $\bar{X_{con}} = N(\mu_{con}, \sigma_{con})$ , $\bar{X_{exp}} = N(\mu_{exp}, \sigma_{exp})$, $\bar{X_{con}} - \bar{X_{exp}} = N(\mu_{con}-\mu_{exp}, \sigma_{con}^{2}/N_{con}+\sigma_{exp}^{2}/N_{exp})$ N = $\frac{(\sigma_{con}^{2}+\sigma_{exp}^{2})*(z_{1-\alpha/2}+z_{1-\beta})^{2}}{\delta^{2}}$ derived based on AA testing. As the risks we are taking increases, $\alpha$ and $\beta$, the number of samples needed for the test reduces. If MDE be small, then we need more samples to detect the rise in the outcome.
